...
class TwistedPuddle(object):
    ...
    def handleOSC(self, message, address):
        ...
        # self.launchpad = [bytearray([0x80,0x80,0x80])]*8
        for i in range(0, self.leds_per_spoke):
            # v = arg[int(i/6)] + arg[int(2*i/6)] + arg[int(3*i/6)]
            shift = int(i * .2)
            v = arg[0 + shift] + arg[1 + shift] + arg[2 + shift]
    ...
    def setLedMapFromSoundVals(self): # magic numbers!
        ...
        new_sparkle_fraction = [];

        for i in range(self.leds_per_spoke):
            value = self.soundVals[self.leds_per_spoke - i - 1]
            ...
            new_sparkle_fraction.append(sparkle_fraction / sparkle_length)

        # print new_sparkle_fraction
        for spoke_num in range(self.nspokes):
            for i in range(self.leds_per_spoke):
                s = random.random()

                bonus = (i < self.leds_per_spoke - 1 and not self.led_map[spoke_num][i]) and (self.location_boost * ((self.led_map[spoke_num][i+1] is match) and 1.0)) * random.random()

                if (s - bonus < new_sparkle_fraction[i]):
        ...
        self.adaptiveThreshold = [max(threshold - self.threshold_decay, value) for threshold, value in zip(self.adaptiveThreshold, self.soundVals)]
