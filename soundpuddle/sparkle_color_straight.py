...
class LEDLoop(object):
	...
	randomr = random.random() * 100
	randomg = random.random() *100
	randomb = random.random()*100
        self.random_color = bytearray([randomg,randomr,randomb]) # g, r, b
    ...
    def setBufferFromLedMap(self):
        for led_idx, led_val in enumerate(self.led_map):
            if (led_val):
                for k in range(3):
                    self.buff[led_idx * 3 + k] = self.random_color
